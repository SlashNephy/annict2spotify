FROM --platform=$BUILDPLATFORM node:18.1.0-bullseye-slim AS cache
WORKDIR /app
...
FROM --platform=$BUILDPLATFORM node:18.1.0-bullseye-slim AS build
WORKDIR /app
...
FROM --platform=$TARGETPLATFORM node:18.1.0-bullseye-slim as runtime
WORKDIR /app
